...
class ReviewRoutes {
    ...
    initRoutes() {
        ...
        const validateModel = check('model').isString().notEmpty().withMessage('Model parameter cannot be empty');
        const validateScore = check('score').isInt({ min: 1, max: 5 }).notEmpty().withMessage('Score must be an integer between 1 and 5');
        const validateComment = check('comment').isString().notEmpty().withMessage('Comment cannot be null or empty');
        const handleValidation = (req: any, res: any, next: any) => this.errorHandler.validateRequest(req, res, next);

        /*this.router.get(
            "/seed",
            (req: any, res: any, next: any) => this.seed.seedReviews()
