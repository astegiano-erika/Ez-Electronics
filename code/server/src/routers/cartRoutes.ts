...
class CartRoutes {
    ...
    initRoutes() {
        ...
        const validateModel = check("model").isString().isLength({ min : 1}).withMessage("Model must be a non-empty string");
        ...
        /**
         * Seeder class used to insert example or initial data into a database. 
        this.router.get(
            "/seed",
            (req: any, res: any, next: any) => this.seed.seedCarts()
                .then(()=> res.status(200).end()) 
