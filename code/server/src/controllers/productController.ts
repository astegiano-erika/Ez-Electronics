...
class ProductController {
    ...
    async registerProducts(model: string, category: string, quantity: number, details: string | null, sellingPrice: number, arrivalDate: string | null) /**:Promise<void> */ {
        const result = await this.dao.registerProduct(model, category, sellingPrice, details, quantity, arrivalDate);
        return result;
     }
    ...
    async changeProductQuantity(model: string, newQuantity: number, changeDate: string | null) /**:Promise<number> */ {
        const result = await this.dao.increaseQuantity(model, newQuantity, changeDate? changeDate : new Date().toISOString().split("T")[0]);
        return result;
     }
    ...
    async sellProduct(model: string, quantity: number, sellingDate: string | null) /**:Promise<number> */ { 
        //const check = (await this.dao.getProductsByModel(model))[0]
        const result = await this.dao.sellProduct(model, quantity, sellingDate? sellingDate : new Date().toISOString().split("T")[0]);
        return result;
    }
    ...
    async getProducts(grouping: string | null, category: string | null, model: string | null) /**Promise<Product[]> */ {
        let result;
        if (grouping == "" || grouping == null) result = await this.dao.getProducts();
        if (grouping == "category") result = await this.dao.getProductsByCategory(category);
        if (grouping == "model") result = await this.dao.getProductsByModel(model);
        return result;
     } 
    ...
    async getAvailableProducts(grouping: string | null, category: string | null, model: string | null) /**:Promise<Product[]> */ { 
        let result;
        if (grouping=="" || grouping==null) result = await this.dao.getAvailableProducts();
        if (grouping == "category") result = await this.dao.getAvailableProductsByCategory(category);
        if (grouping == "model") result = await this.dao.getAvailableProductsByModel(model);
        return result;
    }
    ...
    async deleteAllProducts() /**:Promise <Boolean> */ { 
        let result = await this.dao.deleteProducts();
        return result;
    }
    ...
    async deleteProduct(model: string) /**:Promise <Boolean> */ {
        let result = await this.dao.deleteProduct(model);
        return result;
     }
