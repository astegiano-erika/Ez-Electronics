...
class CartController {
    ...
    async addToCart(user: User, product: string)/*: Promise<Boolean>*/ {
        const result = this.dao.addProductToCart(user.username, product);
        return result;
     }
    ...
    async checkoutCart(user: User) /**Promise<Boolean> */ { 
        ...
        const result = await this.dao.payCurrentCart(user.username, date);
        return result;
    }
    ...
    async getCustomerCarts(user: User) /**Promise<Cart[]> */ { 
        const carts = await this.dao.getHistoryCart(user.username);
        return carts;
    } 
    ...
    async removeProductFromCart(user: User, product: string) /**Promise<Boolean> */ { 
        const result = await this.dao.deleteProductFromCart(user.username, product);
        return result;
    }
    ...
    async clearCart(user: User)/*:Promise<Boolean> */ { 
        const result = await this.dao.deleteCurrentCart(user.username);
        return result;
    }
